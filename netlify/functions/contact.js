...
exports.handler = async function (event) {
  if (event.httpMethod === 'OPTIONS') {
    return { statusCode: 204, body: '', headers: { Allow: 'POST, OPTIONS' } };
  }
  ...
  if (!data.name || !isEmail(data.email) || !data.service || !data.message) {
    return {
      statusCode: 400,
      body: JSON.stringify({ error: 'Please complete the required fields.' }),
      headers: { 'Content-Type': 'application/json' },
    };
  }
  ...
  try {
    ...
    if (!resendResponse.ok) {
      ...
      return {
        statusCode: 502,
        body: JSON.stringify({ error: 'We could not send your message. Please email support@aurorasystems.co.zw directly.' }),
        headers: { 'Content-Type': 'application/json' },
      };
    }
  } catch (error) {
